...
from lib.models import VIBE_Demo, TCMR_Demo, BiCF_Demo
...
def main(args):
    ...
    # load demo video from network or local
    ...
    # Run HMR on each person
    print(f'Running HMR on each tracklet...')
    hmr_time = time.time()
    hmr_results = {}
    for person_id in tqdm(list(tracking_results.keys())):
        ...
        dataloader = DataLoader(dataset, batch_size=args.hmr_batch_size, num_workers=16, drop_last=True)
        ...
        hmr_results[person_id] = output_dict

    del model

    end = time.time()
    fps = num_frames / (end - hmr_time)

    print(f'HMR FPS: {fps:.2f}')
    ...
    joblib.dump(hmr_results, os.path.join(output_path, f"{args.model_name}_output.pkl"))
    ...
    if not args.no_render:
        ...
        frame_results = prepare_rendering_results(hmr_results, num_frames)
        mesh_color = {k: colorsys.hsv_to_rgb(np.random.rand(), 0.5, 1.0) for k in hmr_results.keys()}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str,
                        help='which model: vibe, tcmr, demo_data')
    ...
    parser.add_argument('--hmr_batch_size', type=int, default=512,
                        help='batch size of HMR model')
