...
from lib.core.config import BLVF_DATA_DIR
...
from lib.models import TCMR, BiCF
...
def main(args):
    ...
    #
    output_path = osp.join(f'./output/{args.model_name}')
    Path(output_path).mkdir(parents=True, exist_ok=True)
    image_folder = '/data1/wupeng/myProject/BiCF/internet4'
    num_frames = len(os.listdir(image_folder))
    img_shape = cv2.imread(osp.join(image_folder, '1.jpg')).shape
    ...
    model = BiCF(
        intra_dim=2048,  # clip transformer
        intra_depth=1,
        intra_heads=8,
        intra_dim_head=256,
        intra_mlp_dim=512,
        intra_dropout=0.,
        inter_dim=256,  # view transformer
        inter_depth=1,
        inter_heads=8,
        inter_dim_head=32,
        inter_mlp_dim=64,
    ...
    ckpt = torch.load(pretrained_file, map_location='cuda:0')
    ...
    hmr = hmr().to(device)
    checkpoint = torch.load(osp.join(BLVF_DATA_DIR, 'spin_model_checkpoint.pth.tar'))
    hmr.load_state_dict(checkpoint['model'], strict=False)
    hmr.eval()

    """ Run Algorithm on each person """
    print("\nRunning Algorithm on each person tracklet...")
    tcmr_time = time.time()
    tcmr_results = {}
    ...
    print(f'BiCF FPS: {fps:.2f}')
    total_time = time.time() - total_time
    print(f'Total time spent: {total_time:.2f} seconds (including model loading time).')
    print(f'Total FPS (including model loading time): {num_frames / total_time:.2f}.')

    if args.save_pkl:
        print(f"Saving output results to \'{os.path.join(output_path, 'bicf_output.pkl')}\'.")
        joblib.dump(tcmr_results, os.path.join(output_path, "bicf_output.pkl"))
    ...
    for frame_idx in tqdm(range(len(image_file_names))):
        ...
        # save output frames
        if not os.path.exists(os.path.join('./render_result', f'{args.model_name}')):
            os.makedirs(os.path.join('./render_result', f'{args.model_name}'), exist_ok=True)
        if not os.path.exists(os.path.join('./input_images', f'{args.model_name}')):
            os.makedirs(os.path.join('./input_images', f'{args.model_name}'), exist_ok=True)

        cv2.imwrite(os.path.join('./render_result', f'{args.model_name}', f'{frame_idx:06d}.png'), img)
        cv2.imwrite(os.path.join('./input_images', f'{args.model_name}', f'{frame_idx:06d}.jpg'), input_img)
        # cv2.imwrite(os.path.join('./', f'{frame_idx:06d}.jpg'), img)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='./data/blvf_hmr_data/bicf_model_demo.tar')
