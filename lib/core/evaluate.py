...
class Evaluator():
    def __init__(
        ...
        # self.clip_len = int(clip_len / split_rate)
        # self.batch_size = batch_size * split_rate
        #
        # self.clip_center = int(self.clip_len // 2)
    ...
    def validate(self):
        ...
        J_regressor = torch.from_numpy(np.load(osp.join(CONFIG_DATA_PRETRAINED, 'J_regressor_h36m.npy'))).float()
        ...
        for i, target in enumerate(self.test_loader):
            ...
            with torch.no_grad():
                ...
                inp = target['features']
                preds = self.model(inp, is_train=False, J_regressor=J_regressor)

                n_kp = preds[-1]['kp_3d'].shape[-2]
                pred_j3d = preds[-1]['kp_3d'].reshape(-1, n_kp, 3).cpu().numpy()

                target_j3d = target['kp_3d'].reshape(-1, n_kp, 3).cpu().numpy()
            ...
            summary_string = f'({i + 1}/{len(self.test_loader)}) | batch: {batch_time * 1000:.4}ms | ' \
