...
def compute_accel(joints):
    ...
    # joints: 32*8, 14, 3
    '''modify'''
    joints = joints.reshape(-1, 16, 14, 3)
    velocities = joints[:, 1:] - joints[:, :-1]
    acceleration = velocities[:, 1:] - velocities[:, :-1]
    acceleration_normed = np.linalg.norm(acceleration, axis=3)
    return np.mean(np.mean(acceleration_normed, axis=2), axis=1)

    # velocities = joints[1:] - joints[:-1]
    # acceleration = velocities[1:] - velocities[:-1]
    # acceleration_normed = np.linalg.norm(acceleration, axis=2)
    # return np.mean(acceleration_normed, axis=1)
    # joints = joints.reshape(-1, 8, test14, 3)
    # velocities = joints[:, 1:] - joints[:, :-1]
    # acceleration = velocities[:, 1:] - velocities[:, :-1]
    # acceleration = acceleration.reshape(-1, test14, 3)
    # acceleration_normed = np.linalg.norm(acceleration, axis=2)
    # return np.mean(acceleration_normed, axis=1)

    # joints = joints.reshape(-1, 3, test14, 3)
    # velocities = joints[:, 1:] - joints[:, :-1]
    # acceleration = velocities[:, 1:] - velocities[:, :-1]
    # acceleration = acceleration.reshape(-1, test14, 3)
    # acceleration_normed = np.linalg.norm(acceleration, axis=2)
...
def compute_error_accel(joints_gt, joints_pred, vis=None):
    ...
    Returns:
        error_accel (N-2).
    """
    joints_gt = joints_gt.reshape(-1, 16, 14, 3)
    joints_pred = joints_pred.reshape(-1, 16, 14, 3)
    ...
    # (N-2)x14x3
    # modify dim for acc
    # joints_gt = joints_gt.reshape(-1, 8, test14, 3)
    # accel_gt = joints_gt[:, :-2] - 2 * joints_gt[:, 1:-1] + joints_gt[:, 2:]
    # accel_gt = accel_gt.reshape(-1, test14, 3)
    # # accel_gt = joints_gt[:-2] - 2 * joints_gt[1:-1] + joints_gt[2:]
    # joints_pred = joints_pred.reshape(-1, 8, test14, 3)
    # accel_pred = joints_pred[:, :-2] - 2 * joints_pred[:, 1:-1] + joints_pred[:, 2:]
    # accel_pred = accel_pred.reshape(-1, test14, 3)
    # # accel_pred = joints_pred[:-2] - 2 * joints_pred[1:-1] + joints_pred[2:]

    # joints_gt = joints_gt.reshape(-1, 3, test14, 3)
    # accel_gt = joints_gt[:, :-2] - 2 * joints_gt[:, 1:-1] + joints_gt[:, 2:]
    # accel_gt = accel_gt.reshape(-1, test14, 3)
    # joints_pred = joints_pred.reshape(-1, 3, test14, 3)
    # accel_pred = joints_pred[:, :-2] - 2 * joints_pred[:, 1:-1] + joints_pred[:, 2:]
    # accel_pred = accel_pred.reshape(-1, test14, 3)
...
def align_by_pelvis(joints):
    """
    Assumes joints is test14 x 3 in LSP order.
    Then hips are: [3, 2]
...
def compute_errors(gt3ds, preds):
    ...
    Evaluates on the test14 common joints.
    Inputs:
      - gt3ds: N x test14 x 3
      - preds: N x test14 x 3
    """
    errors, errors_pa = [], []
