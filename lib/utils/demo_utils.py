...
def video_to_images(vid_file, img_folder=None, return_info=False):
    ...
    # else:
    #     return img_folder
    img_folder = '/data1/wupeng/myProject/Now/BiVF_HMR/ava2'
    img_shape = cv2.imread(osp.join(img_folder, '000000.jpg')).shape
    return img_folder, len(os.listdir(img_folder)), img_shape
...
def download_ckpt(outdir='data/blvf_hmr_data', use_3dpw=True):
    os.makedirs(outdir, exist_ok=True)

    if use_3dpw:
        # ckpt_file = 'data/blvf_hmr_data/vibe_model_w_3dpw.pth.tar'
        ckpt_file = 'data/blvf_hmr_data/tcmr_demo_model.pth.tar'
        url = 'https://www.dropbox.com/s/41ozgqorcp095ja/vibe_model_w_3dpw.pth.tar'
        if not os.path.isfile(ckpt_file):
            download_url(url=url, outdir=outdir)
    else:
        ckpt_file = 'data/blvf_hmr_data/vibe_model_wo_3dpw.pth.tar'
